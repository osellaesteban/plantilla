 ...
 */
spiConfig_t spi_conf = {NULL, MASTER, MODE0, SPI_BR, SPI_DMA, NULL};

static spiPort_t ili9341_spi;							/*!< uC SPI port */
static gpioPin_t ili9341_cs, ili9341_dc, ili9341_rst;	/*!< uC GPIO ports to use as CS, DC and RST */
...
void SetChipSelect(uint8_t state)
{
	if(state)
	{
		GPIOSetHigh(ili9341_cs);
	}
	else
	{
		GPIOSetLow(ili9341_cs);
	}
}
...
{
	/* SPI configuration */
	spi_conf.port = spi_port;
	spi_conf.SetCS = SetChipSelect;
	ili9341_spi = spi_port;
	/* GPIOs configuration and initialization */
	ili9341_cs = gpio_cs;
	ili9341_dc = gpio_dc;
	ili9341_rst = gpio_rst;
	gpioConf_t gpio_config[] =
	{
		{gpio_cs, OUTPUT, NONE_RES},
		{gpio_dc, OUTPUT, NONE_RES},
		{gpio_rst, OUTPUT, NONE_RES}
	};
	for (uint8_t i = 0; i < sizeof(gpio_config)/sizeof(gpioConf_t); i++)
	{
		GPIOInit(gpio_config[i]);
	}
	/* CS start in high state */
	GPIOSetHigh(ili9341_cs);

	/* RST must be held low for minimum 10µsec after VCC have been applied */
	DelayUs(10);
	GPIOSetHigh(ili9341_rst);
